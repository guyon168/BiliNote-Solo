...
import { Copy, Download, ArrowRight, Play, ExternalLink, Save, X } from 'lucide-react'
...
import { importNote, updateNote } from '@/services/note'
import VditorEditor from '@/components/VditorEditor.tsx'
...
const remarkPlugins = [[remarkMath, { singleDollarTextMath: true }], gfm]
...
const MarkdownViewer: FC<MarkdownViewerProps> = memo(({ status }) => {
  ...
  useEffect(() => {
    ...
    if (!isMultiVersion) {
      ...
      setSelectedContent(currentTask?.markdown as string)
    } else {
  ...
  const handleSaveEdit = async () => {
    ...
    try {
      // 1. 先回写到后端 note_results/{taskId}.json 的 markdown 字段
      await updateNote(taskId, editContent)

      // 2. 更新前端 Zustand store
      if (isMultiVersion) {
        const updatedVersions = (currentTask.markdown as VersionNote[]).map(v =>
          v.ver_id === currentVerId ? { ...v, content: editContent } : v
        )
        updateTaskContent(taskId, { markdown: updatedVersions })
      } else {
        updateTaskContent(taskId, { markdown: editContent })
      }

      // 3. 立即刷新预览（避免依赖 useEffect 的异步更新）
  ...
  const handleFileChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    reader.onload = async (event) => {
      ...
      try {
        // 1. 先保存到后端文件系统（持久化）
        const res = await importNote(title, content)
        const taskId = res.task_id

        // 2. 创建前端任务并设为完成
        addPendingTask(taskId, 'local', {
          platform: 'local',
          video_url: '',
          quality: 'medium',
          model_name: 'imported',
        ...
        toast.success(`已导入笔记: ${title}`)
      } catch (err) {
        toast.error('导入失败，请检查后端是否运行')
  ...
  return (
    <div className="flex h-screen w-full flex-col overflow-hidden">
      <MarkdownHeader
        ...
        isEditing={isEditing}
        onToggleEdit={currentTask?.status === 'SUCCESS' ? handleToggleEdit : undefined}
        createAt={createTime}
        ...
        <div className="flex flex-1 overflow-hidden bg-white py-2">
          {selectedContent && selectedContent !== 'loading' && selectedContent !== 'empty' ? (
            <>
              ...
              <>
              {isEditing ? (
                /* 编辑模式 - Vditor 分屏编辑器（独立于 ScrollArea 确保高度正确） */
                <div className="flex flex-1 flex-col overflow-hidden">
                  ...
                  <div className="flex items-center gap-2 px-4 py-3 shrink-0 border-t">
