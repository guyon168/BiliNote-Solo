...
# 优先读取 .env 里的外置路径，如果没有配，再回退到代码所在目录下的文件夹
# 1. 读取 .env 中的网络路由前缀和截图输出目录
static_path = os.getenv('STATIC', '/static')
out_dir = os.getenv('OUT_DIR', './static/screenshots') # 👈 刚才报错就是因为缺了这一行

# 2. 获取当前项目的物理根目录（兜底用）
BASE_DIR = os.path.dirname(os.path.abspath(__file__))

# 3. 优先读取 .env 里的外置路径，如果没有配，再回退到代码所在目录下的文件夹
...
# 4. 自动创建目录（无论是本地的还是你外挂的，没有它都会自动建）
