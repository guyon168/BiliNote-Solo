...
# 使用 path_helper 统一获取外置数据目录
from app.utils.path_helper import DATA_ROOT, get_static_dir, get_screenshots_dir

DATA_DIR = DATA_ROOT  # 外置数据根目录（由 DATA_DIR 环境变量决定）
SAVE_PATH = os.path.join(DATA_DIR, "note_output")
IMAGE_BASE_URL = os.getenv("IMAGE_BASE_URL", "/static/screenshots")
STATIC_BASE = get_static_dir()  # 静态文件物理目录
    ...
    def _replace_static_paths_with_absolute(self, content: str) -> str:
        ...
        def repl(match):
            ...
            if img_path.startswith("/static/"):
                # 构建绝对路径（使用 DATA_DIR 而非 BASE_DIR，因为静态文件在外置数据目录中）
